declare module 'verb-nurbs' {
    ...
    export namespace eval
    {
    	...
    	{
    		static knotMultiplicities(knots:number[]) : eval.KnotMultiplicity[];
    ...
    export namespace core
    {
    	...
    	{
    		constructor(points:core.KdPoint<T>[], distanceFunction:(arg0:number[], arg1:number[]) => number);
    		nearest(point:number[], maxNodes:number, maxDistance:number) : core.Pair<core.KdPoint<T>, number>[];
    	}
    	...
    	{
    		static uncmin(f:(arg:number[]) => number, x0:number[], tol?:number, gradient?:(arg:number[]) => number[], maxit?:number) : core.MinimizationResult;
    	}
