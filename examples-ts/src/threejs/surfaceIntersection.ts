...
export function init(container: HTMLElement) {
  ...
  const srf1 = verb.geom.NurbsSurface.byKnotsControlPointsWeights(degree, degree, knots, knots, pts);

  const srf2 = verb.geom.NurbsSurface.byCorners(
    [50, -50, 3], [50, 0, 3], [0, 0, 3], [0, -50, 5]
  );

  addMesh(ctx.scene, surfaceToBufferGeometry(srf1));
  addMesh(ctx.scene, surfaceToBufferGeometry(srf2));

  const intersections = verb.geom.Intersect.surfaces(srf1, srf2, 1e-6);
  intersections.forEach((curve) => {
    addCurve(ctx.scene, curveToBufferGeometry(curve));
  });
